...
def read_images( filenames, domain=None, image_size=64):
    images = []
    for fn in filenames:
        image_dir = os.path.join(face_3d_path, fn)
...
def get_faces_3d(test=False, half=None):
    ...
    for current_id in current_ids:
        ar_image_paths += groups.get_group(current_id)['image_path'].tolist()

    #df['file_name'] = df['image_path'].map(lambda x: x)
    for image_path in ar_image_paths:
        angle = image_path.split('_')[4]
        if half is None:
            image_paths.append(image_path)
        if half == 'first' and angle !='0':
            image_paths.append(image_path)
        if half == 'last' and angle == '0':
            for i in range(8):
                image_paths.append(image_path)
    #print(image_paths)
    '''
    df['id'] = df['image_path'].map(lambda x: x.split('/')[-1][:20])
    df['angle'] = df['image_path'].map(lambda x: x.split('_')[4])
    #angle_list = df['angle'].tolist()
    unique_ids = df['id'].unique()
    unique_angle = df['angle'].unique()
    #print(unique_ids)
    a = unique_angle[:6]
    a = np.append(a, unique_angle[7:])
    
    if not test: #change this
        if half is None:
            current_ids = unique_ids[:8]
        if half == 'first':
